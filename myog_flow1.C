...
4 particle correlation should be -vn^4    (there seems to be a sign wrong?)
  ...
  // we will define our histograms here
   ...
   // make an event **************** set V2 and PHI here
     ...
     //cout << "NPARTICLES = " << nparticles << endl << endl;
...
     PSI2h->Fill(PSI2Event);
     ...
     for(int i=0; i<nparticles;i++){
       ...
       ytrack[i]=0.; // set all pt to 1 GeV now
	 ...
	 cout << "ncdphisum = " << ncdphisum << endl;
	 //cout << "cdphisum = " << cdphisum << endl;
     ...
     //cdphisum_av=cdphisum/ncdphisum;

     //cdphisumtot = cdphisumtot + cdphisum_av;
     cdphisumtot+=cdphisum/ncdphisum;
     cdphi3sumtot+=cdphi3sum/ncdphi3sum;
     cdphi4sumtot+=cdphi4sum/ncdphi4sum;

     //reset all values after the event has completed
     //cdphisum=0;
     //ncdphisum=0;
     //cdphi3sum=0;
     //ncdphi3sum=0;
   ...
   cout<<" FINAL event plane reslution="<<res_eventplane<<endl;
   ...
   cout<<" v2 from cumulants="<<sqrt(cdphisumtot/nevents)<<endl;
   cout<<" 3 particle cumulant="<<cdphi3sumtot/nevents<<endl;
   cout<<" 4 particle cumulant="<<cdphi4sumtot/nevents<<endl;
   ...
   cout<<" FINAL v"<<nharmonic<<" from Event plane="<<v2obs<<" nharmonic="<<nharmonic<<endl;
    ...
    cout<<" from 2 particle cumulant: v"<<nharmonic<<"="<<sqrt(cdphisumtot/nevents)<<endl;
    cout<<" from 4 particle cumulant: v"<<nharmonic<<"="<<sqrt(sqrt(cdphi4sumtot/nevents))<<" using 4throot(-c4)="<<sqrt(sqrt(-cdphi4sumtot/nevents))<<endl;
    double cum3=0;
    if(nharmonic==1)cum3=v1*v1*v2;
    if(nharmonic==2)cum3=v2*v2*v4;
    cout<<" nharmonic="<<nharmonic<<", 3 particle cumulant="<<cdphi3sumtot/nevents<<" we expect "<<cum3<<" i.e. v"<<nharmonic<<"*v"<<nharmonic<<"*v"<<2*nharmonic<<endl;
    //---------------------
