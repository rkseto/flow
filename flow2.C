...
4 particle correlation should be -vn^4    (there seems to be a sign wrong?)
  ...
  // we will define our histograms here
   ...
   int nharmonic=2;  //2
   int nevents=1000; //1000, for nharmonic=4 need 4000
   ...
   // make an event **************** set V2 and PHI here
     ...
     double PSI2found=atan(Q2y/Q2x)/nharmonic;
     ...
     // see PRC 83 044913
     double b2b =(Qnx[nharmonic-1]*Qnx[nharmonic-1]+Qny[nharmonic-1]*Qny[nharmonic-1]-nparticles)/nparticles/(nparticles-1);// b2b=<2>
     double q44=(Qnx[nharmonic-1]*Qnx[nharmonic-1]+Qny[nharmonic-1]*Qny[nharmonic-1])*(Qnx[nharmonic-1]*Qnx[nharmonic-1]+Qny[nharmonic-1]*Qny[nharmonic-1]); // Q_nharmonic^4  
     double q82=Qnx[nharmonic-1]*Qnx[nharmonic-1]+Qny[nharmonic-1]*Qny[nharmonic-1];// Q_2*nharmonic^2
     double q42=Qnx[nharmonic-1]*Qnx[nharmonic-1]+Qny[nharmonic-1]*Qny[nharmonic-1];// Q_nharmonic^2
     double q8q4q4= Qnx[2*nharmonic-1]*Qnx[nharmonic-1]*Qnx[nharmonic-1]-Qnx[2*nharmonic-1]*Qny[nharmonic-1]*Qny[nharmonic-1]+Qny[2*nharmonic-1]*Qnx[nharmonic-1]*Qny[nharmonic-1];// Re[Q_2*nharmonic*Q_nharmonicstar*Q_nharmonicstar]
     double b4b =(q44+q82-2*q8q4q4)/
       (nparticles*(nparticles-1)*(nparticles-2)*(nparticles-3)
	-2*(2*(nparticles-2)*q42-nparticles*(nparticles-3)))/
       (nparticles*(nparticles-1)*(nparticles-2)*(nparticles-3));// b2b=<4>
     CF2+=b2b;
     CF4+=b4b;
   ...
   // calculate CFs
   CF2=CF2/float(nevents);
   CF4=CF4/float(nevents);
   CF4=CF4-2*CF2*CF2;
   ...
   cout<<" CF2="<<CF2<<" sqrt(CF2):vn{2}="<<sqrt(CF2)<<" CF4="<<CF4
       <<" sqrt(sqrt(-CF4)):vn{4}="<<sqrt(sqrt(-CF4))<<endl;
   ...
   //
   TFile *myfile = new TFile("flow2.root","RECREATE");
