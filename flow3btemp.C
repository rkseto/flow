// correct res_eventplane (sqrt(2)*sqrt(res_eventplane))
  ...
  // we will define our histograms here
   ...
   // make an event **************** set V2 and PHI here
	 ...
	 Q2ysub2=Q2ysub2+sin(nharmonic*phitrack[i]);
       ...
       if(i<nthird){
	 Q2xsub1=Q2xsuba+cos(nharmonic*phitrack[i]);
	 Q2ysub1=Q2ysuba+sin(nharmonic*phitrack[i]);
       }else if(i<2*nthird){
	 Q2xsub2=Q2xsubb+cos(nharmonic*phitrack[i]);
	 Q2ysub2=Q2ysubb+sin(nharmonic*phitrack[i]);
       }else if(i<3*nthird){
       }else{
	 Q2xsub2=Q2xsubc+cos(nharmonic*phitrack[i]);
	 Q2ysub2=Q2ysubc+sin(nharmonic*phitrack[i]);
       }
   ...
   TF1 *myfit = new TF1("myfit","[0]*(1.0+2*[1]*cos(1*x) + 2*[2]*cos(2*x)+ 2*[3]*cos(3*x)+ 2*[4]*cos(4*x)+ 2*[5]*cos(5*x) )", -PI, PI);
    ...
    cout<<" FINAL v"<<nharmonic<<" from Event plane="<<v2obs<<" nharmonic="<<nharmonic<<" v2 corrected for resolution="<<v2obs/res_eventplane<<" v2/res_eventplane3sub="<<v2obs/res_eventplane3sub<<endl;
   ...
   //
   TFile *myfile = new TFile("flow3.root","RECREATE");
