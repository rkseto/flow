...
4 particle correlation should be -vn^4  
  ...
  TH1D *reseventplaneh = new TH1D("reseventplaneh","event plane reslution event",2000,0.0,1.00);
   ...
   //   gBenchmark->Start("flow1");
                  //FXT3GeV//200GeV //test
   double v1=0.10; //0.02   // 0.05; // 0.1
   double v2=0.20; //0.05   //0.15;  // 0.2
   double v3=0.15; //0.01  //0.07;  //0.15
   double v4=0.05; //0.02  //0.03;  //0.05
   ...
   cout<<" nevents(1000) max is 100000: ";
   cin>>nevents;
   ...
   int nparticlesmin=6;
   int ncount=100;
   if(nevents>=100000)ncount=100000;
   double PSI2=0.;
   ...
   int nparticles=0;
   double phitrack[100000];
   double pttrack[100000];
   double ytrack[100000];
   int nparticles_previous=0;
   double phitrack_previous[100000];
   double pttrack_previous[100000];
   double ytrack_previous[100000];
   ...
   // make an event **************** set V2 and PHI here
     ...
     for(int i=0; i<nparticles;i++){
       pttrack[i]=gRandom->Exp(0.250);  // 250 MeV tau
       pttrack[i]+=0.180;

       //       flowf->SetParameters(PSI2Event,v1,v2*pttrack[i],v3,v4);
   ...
   //
   TFile *myfile = new TFile("flow4.root","RECREATE");
