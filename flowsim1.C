...
void flowsim1(int neventsmax=100,  int nharmonic=2, double ymaxcut=4.
) {
  /*
...
4 particle correlation should be -vn^4  
  ...
  while(std::getline(infile,line)){
    ...
    for(int ipart=0; ipart<nparticles; ipart++){
      ...
      pyh->Fill(double(ppy));
      pzh->Fill(double(ppz));
      if(abs(ytrack)<ymaxcut)phih->Fill(phitrack);
      etah->Fill(etatrack);
    ...
    int ncdphisum=0;
     ...
     for(int i=0; i<nparticles;i++){
       ...
       if(abs(ytrackar[i])>ymaxcut)continue;
	 ...
	 if(abs(ytrackar[j])>ymaxcut)continue;
	 ...
	 for(int k=j+1; k<nparticles; k++){
	   if(abs(ytrackar[k])>ymaxcut)continue;
	   if(!doloop) break;
	   
	   for(int l=k+1; l<nparticles; l++){
	     if(abs(ytrackar[l])>ymaxcut)continue;
	 ...
	 for(int j=i+1; j<nparticles_previous;j++){
	   if(abs(ytrack_previousar[j])>ymaxcut)continue;
  ...
  // Open a ROOT file and save the formula, function and histogram
   //
   TFile *myfile = new TFile("flowsim1.root","RECREATE");
